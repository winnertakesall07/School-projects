 ...
 *   - Turing Machine (computer simulation)
 */
public class Pattern {
    ...
    /**
     * Turing Machine — a large Game-of-Life pattern that implements a
     * universal computer (Turing-complete) entirely within Conway's GoL rules.
     * Loaded from the accompanying "turingmaschine.txt" RLE file.
     *
     * The pattern (4086 × 5012 cells) is annotated with two regions:
     *   INPUT  — the initial signal/tape data along the top-left diagonal.
     *   OUTPUT — the region where the head writes its result (bottom-right area).
     *
     * Returns {@code null} if the file cannot be read.
     */
    public static Pattern turingMachine() {
        // Look for the RLE file next to the running class files
        String[] candidates = {
            "turingmaschine.txt",
        ...
        for (String path : candidates) {
            ...
            if (f.exists()) {
                try {
                    Pattern base = fromRleFile("Turing Machine (computer)", f);
                    int[] bounds = base.getBounds();
                    int W = bounds[0], H = bounds[1];
                    // Input:  top-left diagonal band — the initial tape-signal stream.
                    // Output: bottom-right area    — where the machine writes results.
    ...
    /**
     * Turing Machine V2 — a second, distinct Game-of-Life Turing-machine
     * pattern loaded from the accompanying "turingmaschineV2" RLE file.
     *
     * Returns {@code null} if the file cannot be read.
     */
    public static Pattern turingMachineV2() {
        String[] candidates = {
            "turingmaschineV2",
            "ConwaysGameOfLife_v6/turingmaschineV2",
            "../turingmaschineV2"
        };
        for (String path : candidates) {
            File f = new File(path);
            if (f.exists()) {
                try {
                    Pattern base = fromRleFile("Turing Machine V2 (computer)", f);
                    int[] bounds = base.getBounds();
                    int W = bounds[0], H = bounds[1];
                    String[] labels = { "INPUT", "OUTPUT" };
                    int[][] boxes   = {
     ...
     * Any uppercase letter is treated as alive; '.' is dead; '$' ends a row;
     * '!' ends the pattern; digits act as run-length prefixes.
     */
