...
public class GameOfLifeBoard {
    ...
    public void nextGeneration() {
        ...
        if (!MULTI_CORE || sz < PARALLEL_THRESHOLD) {
                ...
                for (int dy = -1; dy <= 1; dy++)
                    for (int dx = -1; dx <= 1; dx++)
                        if (dx != 0 || dy != 0) {
                            ...
                            Integer old = counts.get(nk);
                            counts.put(nk, old == null ? 1 : old + 1);
                        }
            }
                ...
                (map, ck) -> {
                    ...
                    for (int dy = -1; dy <= 1; dy++)
                        for (int dx = -1; dx <= 1; dx++)
                            if (dx != 0 || dy != 0) {
                                long nk = key(cx + dx, cy + dy);
                                Integer old = map.get(nk);
                                map.put(nk, old == null ? 1 : old + 1);
                            }
                },
            ...
            // A simple loop is usually faster than an additional parallel stream.
            next = new HashSet<>((int)(counts.size() / 0.75f) + 1);
            for (Map.Entry<Long, Integer> e : counts.entrySet()) {
                int n = e.getValue();
                long k = e.getKey();
