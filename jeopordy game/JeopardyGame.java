...
/**
 * ============================================================
 *  JeopardyGame - Katholische Religion, Q12 München
 *  BlueJ-kompatibles Jeopardy-Spielbrett
 * ============================================================
 *
 *  ZUM STARTEN:
 *    Öffne diesen Ordner in BlueJ → Rechtsklick auf JeopardyGame
 *    → main(String[]) → OK
 *
 *  FRAGEN ANPASSEN:
 *    Scrolle nach unten zur Methode initializeQuestions() und
 *    ersetze die Platzhalter-Fragen mit deinen eigenen Inhalten.
 *    Jede Frage wird so erstellt:
 *      questions[KATEGORIE][PUNKTZAHL] = new Question("Frage", "Antwort");
 *    Kategorien 0-4, Punktzahlen 0-4 (entspricht 100-500 Punkte).
 *
 *  SPIELERNAMEN ÄNDERN:
 *    Bearbeite das Array PLAYER_NAMES weiter unten.
 * ============================================================
 */
public class JeopardyGame extends JFrame {

    // ── Spielernamen (hier anpassen, oder zur Laufzeit über den Button) ──
    ...
    private static final Color C_CELL_HOVER = new Color(10, 20,  190);
    private static final Color C_CELL_DONE  = new Color(0,   0,   60);
    private static final Color C_GOLD       = new Color(255, 215,   0);
    private static final Color C_WHITE      = Color.WHITE;
    private static final Color C_GREY       = new Color(90, 90, 110);
    private static final Color C_RED_BTN    = new Color(140,  0,   0);
    ...
    private void initializeQuestions() {
        ...
        questions[3][4] = new Question(
            "Was macht Nietzsche mit dem Satz <<Gott ist tot>> - feiert er den Tod Gottes?",
        ...
        questions[4][0] = new Question(
            "Welche Folge hat es laut Nietzsche, wenn man versucht, Moral ohne Gott beizubehalten?",
    ...
    private JPanel buildTitleBar() {
        ...
        title.setFont(new Font("Impact", Font.BOLD, 68));
    ...
    private JPanel buildBoard() {
        int rows = 1 + POINTS.length;          // header row + point rows
    ...
    private JButton makeQuestionCell(final int col, final int row) {
        ...
        btn.setFont(new Font("Impact", Font.BOLD, 36));
    ...
    private JPanel buildScoreBar() {
        ...
        for (int i = 0; i < 3; i++) {
            scoreLabels[i] = makeScoreLabel(i);
            bar.add(makeScorePanel(i));
        }
        return bar;
    }

    private JLabel makeScoreLabel(int idx) {
        JLabel lbl = new JLabel(scoreHtml(idx), SwingConstants.CENTER);
        lbl.setFont(new Font("Arial", Font.BOLD, 18));
        lbl.setForeground(C_WHITE);
        return lbl;
    }

    /** Panel with score label + +100/-100 adjustment buttons. */
    private JPanel makeScorePanel(final int idx) {
        JPanel p = new JPanel(new BorderLayout(4, 0));
        p.setBackground(C_CELL);
        p.setBorder(BorderFactory.createLineBorder(C_GOLD, 2));

        scoreLabels[idx] = makeScoreLabel(idx);
        p.add(scoreLabels[idx], BorderLayout.CENTER);

        // Small +/- buttons to manually adjust score
        JPanel adj = new JPanel(new GridLayout(1, 2, 2, 0));
        adj.setBackground(C_CELL);
        adj.setBorder(BorderFactory.createEmptyBorder(4, 4, 4, 4));
        ...
        p.setPreferredSize(new Dimension(230, 60));
        return p;
    }
    ...
    private void openQuestion(int col, int row) {
        ...
        QuestionDialog dlg = new QuestionDialog(
                this, questions[col][row], POINTS[row], sound, PLAYER_NAMES);
        dlg.setVisible(true);
    ...
    private void markAnswered(int col, int row) {
        ...
        btn.setBorder(BorderFactory.createLineBorder(new Color(0, 0, 50), 1));
        btn.setEnabled(false);
    }
    ...
    private String scoreHtml(int idx) {
        return "<html><center><b>" + PLAYER_NAMES[idx] + "</b><br>"
    ...
    private void showWinnerScreen() {
        ...
        JOptionPane.showMessageDialog(this,
                ...
                + PLAYER_NAMES[0] + ":&nbsp;$" + scores[0] + "<br>"
                + PLAYER_NAMES[1] + ":&nbsp;$" + scores[1] + "<br>"
                + PLAYER_NAMES[2] + ":&nbsp;$" + scores[2]
                + "</font></center></html>",
    ...
    private void editNames() {
        ...
        for (int i = 0; i < 3; i++) {
            String input = (String) JOptionPane.showInputDialog(this,
                    ...
                    JOptionPane.PLAIN_MESSAGE, null, null, PLAYER_NAMES[i]);
            newNames[i] = (input == null || input.trim().isEmpty())
                    ? PLAYER_NAMES[i] : input.trim();
        }
        for (int i = 0; i < 3; i++) PLAYER_NAMES[i] = newNames[i];
        refreshScores();
    }
    ...
    public static void main(String[] args) {
        // Schönes Look & Feel des Systems verwenden
        try {
            UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
        } catch (Exception ignored) {}

        SwingUtilities.invokeLater(() -> new JeopardyGame());
    }
}
