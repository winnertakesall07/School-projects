 ...
 *   - Obstacle-aware steering: rotate direction when direct path is blocked
 *   - Periodic strafing: enemies vary their approach rather than charging in a straight line
 */
public class Enemy {
    ...
    private float dodgeDirY   = 0f;
    private static final int   DODGE_DURATION   = 22;
    private static final float DODGE_RANGE      = 180f;
    private static final float DODGE_THREAT_DOT = 0.72f; // cos ~44°

    // ── Strafe state ──────────────────────────────────────────────────────────
    private int   strafeTimer     = 0;
    private int   strafeInterval;  // countdown until next strafe
    private float strafeDirX      = 0f;
    private float strafeDirY      = 0f;
    private static final int STRAFE_DURATION = 30;
    ...
    public Enemy(int startX, int startY, int chapter, SubType subType) {
        ...
        strafeInterval      = 80  + RNG.nextInt(120);
    }
     ...
     * @param covers         cover objects for collision / sight checks
     * @param allies         list of ally soldiers (can be null / empty)
     * @param playerBullets  active player bullets used for dodge detection
    ...
    public Bullet update(float targetX, float targetY,
        ...
        if (strafeInterval    > 0) strafeInterval--;

        // ── Bullet dodge detection ────────────────────────────────────────────
        if (dodgeTimer <= 0 && playerBullets != null) {
            for (Bullet b : playerBullets) {
                float bdx  = getCenterX() - b.x;
                float bdy  = getCenterY() - b.y;
                float dist = (float) Math.sqrt(bdx * bdx + bdy * bdy);
                if (dist < DODGE_RANGE) {
                    // Normalise bullet velocity and check if heading toward us
                    float bvLen = (float) Math.sqrt(b.getDx() * b.getDx() + b.getDy() * b.getDy());
                    if (bvLen > 0.001f) {
                        float bnx = b.getDx() / bvLen;
                        float bny = b.getDy() / bvLen;
                        // Dot product of bullet dir and direction TO this enemy
                        float dot = bnx * (bdx / dist) + bny * (bdy / dist);
                        if (dot > DODGE_THREAT_DOT) {
                            // Perpendicular sidestep (choose a random side)
                            dodgeDirX = -bny;
                            dodgeDirY =  bnx;
                            if (RNG.nextBoolean()) { dodgeDirX = -dodgeDirX; dodgeDirY = -dodgeDirY; }
        ...
        // ── Periodic strafe update ────────────────────────────────────────────
        if (strafeInterval <= 0 && strafeTimer <= 0) {
            // Compute a perpendicular to the player direction
            float pdx  = targetX - getCenterX();
            float pdy  = targetY - getCenterY();
            float pLen = (float) Math.sqrt(pdx * pdx + pdy * pdy);
            if (pLen > 0.01f) {
                strafeDirX = -pdy / pLen;
                strafeDirY =  pdx / pLen;
                if (RNG.nextBoolean()) { strafeDirX = -strafeDirX; strafeDirY = -strafeDirY; }
            }
            strafeTimer    = STRAFE_DURATION;
            strafeInterval = 90 + RNG.nextInt(140);
        }
        ...
        if (lowHP && (subType == SubType.RANGED || subType == SubType.COMMANDER)) {
            ...
            if (targetCover != null) {
                ...
                if (cd > SIZE * 0.6f) {
                    float[] steered = steerDir(cdx / cd, cdy / cd, covers);
                    moveStep(steered[0], steered[1], covers, worldW, worldH);
                    inCover = false;
                } else {
                    inCover = true;
                }
            ...
            switch (subType) {
                case MELEE: {
                    if (dist > SIZE * 0.8f) {
                        float[] moveDir = computeMoveDir(dx / dist, dy / dist, covers);
                        moveStep(moveDir[0], moveDir[1], covers, worldW, worldH);
                    }
                    break;
                }
                ...
                case COMMANDER: {
                    if (dist > SIZE * 0.8f) {
                        float[] steered = steerDir(dx / dist, dy / dist, covers);
                        moveStep(steered[0], steered[1], covers, worldW, worldH);
                    }
                ...
                case RANGED: {
                    ...
                    if (dist > preferred + 45) {
                        baseDir = steerDir(dx / dist, dy / dist, covers);
                        moveStep(baseDir[0], baseDir[1], covers, worldW, worldH);
                ...
                case FLANKER: {
                    ...
                    if (dist > SIZE * 0.9f && ftd > 5f) {
                        float[] steered = steerDir(ftdx / ftd, ftdy / ftd, covers);
                        moveStep(steered[0], steered[1], covers, worldW, worldH);
                    }
                    break;
                }
    ...
    /**
     * Compute the final movement direction, blending steering + dodge + strafe.
     * Steering already points away from obstacles; dodge and strafe are layered on top.
     */
    private float[] computeMoveDir(float wantDirX, float wantDirY, List<CoverObject> covers) {
        float[] steered = steerDir(wantDirX, wantDirY, covers);
        float rx = steered[0], ry = steered[1];

        // Apply dodge override when active
        if (dodgeTimer > 0) {
            rx = dodgeDirX;
            ry = dodgeDirY;
        }

        // Blend in strafe when active (and not dodging)
        if (strafeTimer > 0 && dodgeTimer <= 0) {
            float blend = 0.45f;
            rx = rx * (1f - blend) + strafeDirX * blend;
            ry = ry * (1f - blend) + strafeDirY * blend;
        }

        float len = (float) Math.sqrt(rx * rx + ry * ry);
        if (len < 0.001f) return steered;
        return new float[]{ rx / len, ry / len };
    }

    /**
     * Obstacle-aware steering: if the direct path ahead is blocked by a cover
     * object, rotate the direction in increments until a clear path is found.
     */
    private float[] steerDir(float dirX, float dirY, List<CoverObject> covers) {
        if (!isPathBlocked(dirX, dirY, covers)) return new float[]{ dirX, dirY };

        float[] angles = { 25f, -25f, 50f, -50f, 80f, -80f, 110f, -110f, 140f, -140f, 180f };
        for (float deg : angles) {
            ...
            if (!isPathBlocked(rx, ry, covers)) return new float[]{ rx, ry };
        }
        return new float[]{ dirX, dirY }; // fallback
    }

    /** True if moving ~60 px in (dirX, dirY) from current centre would enter a cover object. */
    private boolean isPathBlocked(float dirX, float dirY, List<CoverObject> covers) {
        float lookAhead = 60f;
        float nx = getCenterX() + dirX * lookAhead - SIZE / 2f;
        float ny = getCenterY() + dirY * lookAhead - SIZE / 2f;
        for (CoverObject c : covers) {
            if (c.overlapsEntity(nx, ny, SIZE)) return true;
        }
        return false;
    }
    ...
    private void moveStep(float dirX, float dirY,
        ...
        // When dodging, allow extra lateral speed
        float spd = (dodgeTimer > 0) ? speed * 1.25f : speed;
        float newX = x + dirX * spd;
        float newY = y + dirY * spd;
    ...
    /**
     * Externally applied push (used for enemy-enemy separation by GamePanel).
     * Same collision resolution as moveStep but no speed multiplier.
     */
    public void push(float dx, float dy, List<CoverObject> covers, int worldW, int worldH) {
        float newX = x + dx;
        float newY = y + dy;
        for (CoverObject c : covers) {
            if (c.overlapsEntity(newX, newY, SIZE)) {
                if (!c.overlapsEntity(newX, y, SIZE)) newY = y;
                else if (!c.overlapsEntity(x, newY, SIZE)) newX = x;
                else { newX = x; newY = y; }
                break;
            }
        }
    ...
    public void draw(Graphics2D g) {
        ...
        // Dodge indicator (brief flash when dodging)
        if (dodgeTimer > 0) {
            g.setColor(new Color(80, 180, 255, 140));
            g.fillRect(ex, ey - 12, SIZE, 4);
        }
