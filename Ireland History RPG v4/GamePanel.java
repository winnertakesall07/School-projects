...
public class GamePanel extends JPanel implements Runnable {
    ...
    private void updateTitle() {
        if (keys.enterPressed || keys.key1Pressed) {
            keys.enterPressed = false; keys.key1Pressed = false; startChapter(0);
        } else if (keys.key2Pressed) {
            keys.key2Pressed = false; startChapter(1);
        } else if (keys.key3Pressed) {
            keys.key3Pressed = false; startChapter(2);
        }
    }

    private void updateStory() {
        if (keys.mouseClickPressed || keys.enterPressed) {
            keys.mouseClickPressed = false;
    ...
    private void updatePlaying() {
        ...
        // ── Enemy separation — push stacked enemies apart ─────────────────────
        for (int i = 0; i < enemies.size(); i++) {
            Enemy ei = enemies.get(i);
            for (int j = i + 1; j < enemies.size(); j++) {
                Enemy ej = enemies.get(j);
                float ddx = ei.getCenterX() - ej.getCenterX();
                float ddy = ei.getCenterY() - ej.getCenterY();
                float dist = (float) Math.sqrt(ddx * ddx + ddy * ddy);
                float minDist = Enemy.SIZE * 0.85f;
                if (dist < minDist && dist > 0.01f) {
                    float overlap = (minDist - dist) / 2f;
                    float nx = ddx / dist * overlap;
                    float ny = ddy / dist * overlap;
                    ei.push( nx,  ny, covers, Camera.WORLD_WIDTH, Camera.WORLD_HEIGHT);
                    ej.push(-nx, -ny, covers, Camera.WORLD_WIDTH, Camera.WORLD_HEIGHT);
                }
            }
        }
    ...
    private void spawnEGroup(int count, Enemy.SubType sub, int chapter,
        ...
        for (int i = 0; i < count; i++) {
            ...
            int[] safe = findValidSpawn(ex, ey, Enemy.SIZE);
            enemies.add(new Enemy(safe[0], safe[1], chapter, sub));
        }
    }
    ...
    private boolean isInsideCover(int ex, int ey, int entitySize) {
        for (CoverObject c : covers) {
            if (c.overlapsEntity(ex, ey, entitySize)) return true;
        }
        return false;
    }
    ...
    private void drawTitle(Graphics2D g) {
        ...
        drawCentered(g, "\u00c9ire\u2019s Stand", SCREEN_HEIGHT / 2 - 170);

        g.setFont(new Font("Serif", Font.ITALIC, 26));
        g.setColor(new Color(175, 210, 175));
        drawCentered(g, "Ireland\u2019s Struggle for Freedom", SCREEN_HEIGHT / 2 - 112);

        g.setFont(new Font("Serif", Font.PLAIN, 42));
        g.setColor(new Color(55, 155, 55));
        drawCentered(g, "\u2618 \u2618 \u2618", SCREEN_HEIGHT / 2 - 58);

        // Chapter select panel
        int panelX = SCREEN_WIDTH / 2 - 220, panelY = SCREEN_HEIGHT / 2 - 20;
        int panelW = 440, panelH = 130;
        g.setColor(new Color(10, 28, 10, 210));
        ...
        g.setColor(new Color(210, 190, 100));
        drawCentered(g, "— Select Chapter —", panelY + 22);

        String[][] chOptions = {
            { "[1]", "Chapter I",   "Norman Conquest  \u2022  1169 A.D." },
            { "[2]", "Chapter II",  "British Rule  \u2022  1803 A.D." },
            { "[3]", "Chapter III", "Easter Rising  \u2022  1916 A.D." }
        };
        g.setFont(new Font("SansSerif", Font.PLAIN, 13));
        for (int i = 0; i < 3; i++) {
            int rowY = panelY + 44 + i * 26;
            g.setColor(new Color(180, 220, 180));
            g.drawString(chOptions[i][0] + "  " + chOptions[i][1] + ":", panelX + 30, rowY);
            g.setColor(new Color(160, 160, 160));
            g.drawString(chOptions[i][2], panelX + 170, rowY);
        }

        g.setFont(new Font("SansSerif", Font.BOLD, 13));
        g.setColor(new Color(200, 200, 200));
        drawCentered(g, "Press 1, 2 or 3 to choose  \u2014  ENTER starts from Chapter I", SCREEN_HEIGHT / 2 + 120);
        ...
        drawCentered(g, "v4: Chapter select \u2022 Click-to-advance story \u2022 Smarter enemies \u2022 No enemy stacking", SCREEN_HEIGHT - 22);
    }
    ...
    private void drawStory(Graphics2D g) {
        ...
        String prompt = storyManager.hasMorePages()
                ? "Click to continue \u25ba  (or press ENTER)"
                : "Click to begin the battle \u2694  (or press ENTER)";
